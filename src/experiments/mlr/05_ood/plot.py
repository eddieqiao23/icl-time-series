...
def main():
 ...
 args.output_dir.mkdir(parents=True,exist_ok=True); labels={'transformer':'Transformer','assignment_oracle':'Assignment oracle','known_pool_uniform':'Known pool (uniform prior)','em_ridge_K1':'EM ridge (K=1)','em_ridge_K2':'EM ridge (K=2)','em_ridge_K3':'EM ridge (K=3)','em_ridge_K4':'EM ridge (K=4)','ridge_current':'Current-task ridge','ridge_history':'History ridge'}
 for family,xlabel in [('similarity','Coefficient cosine similarity'),('imbalance','Majority probability'),('hierarchy','Hierarchy dispersion')]:
  ...
  for method in methods:
   ...
   ax.plot([r['value'] for r in points],[r['mean_mse'] for r in points],marker='o',label=labels[method])
  if family=='imbalance': ax.set_yscale('log')
 ...
 for method in methods:
  ...
  ax.plot([r['value'] for r in points],[r['mean_mse'] for r in points],marker='o',label=labels.get(method,method.replace('_',' ')))
 ax.set_yscale('log')
