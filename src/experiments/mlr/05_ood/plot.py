...
def main():
 ...
 for (family,value,method),vals in sorted(grouped.items()):
  a=np.asarray(vals); se=a.std(ddof=1)/np.sqrt(len(a)); summary.append({'family':family,'value':value,'method':method,'mean_mse':a.mean(),'stderr':se,'ci95_low':max(a.mean()-1.96*se,0),'ci95_high':a.mean()+1.96*se,'num_pools':len(a)})
 with args.summary.open('w',newline='') as f: w=csv.DictWriter(f,fieldnames=list(summary[0]),lineterminator='\n'); w.writeheader(); w.writerows(summary)
 args.output_dir.mkdir(parents=True,exist_ok=True); labels={'transformer':'Transformer','known_pool':'Known pool','em_ridge_K2':'EM ridge (K=2)','em_ridge_K3':'EM ridge (K=3)','ridge_current':'Current-task ridge','ridge_history':'History ridge'}
 for family,xlabel in [('similarity','Coefficient cosine similarity'),('imbalance','Majority probability'),('hierarchy','Hierarchy dispersion')]:
  fig,ax=plt.subplots(figsize=(7.5,4.8)); methods=['transformer','known_pool','em_ridge_K2' if family!='hierarchy' else 'em_ridge_K3','ridge_current','ridge_history']
 ...
 methods=['transformer','known_pool','em_ridge_K1','em_ridge_K2','em_ridge_K3','em_ridge_K4']
 for method in methods:
